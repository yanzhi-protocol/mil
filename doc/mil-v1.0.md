...
Archived memory file schema:

Each file under these directories is a JSON object:
...
```json
{
  ...
  "memory_type": "string (one of: factual, emotional, experiential, relational, cultural)",
  "scene_type": "string (second-level directory name)",
  "subcategory": "string (optional, deeper directory name)",
  "summary": "string (human-readable summary, max 500 characters)",
  "details": "object (key-value data, e.g., emotions, decisions, lessons learned)",
...
1. Intent and entity extraction: Identify key topics, emotions, and references (e.g., “argument with friend”, “interview nervousness”).
2. Path assembly: Map extracted clues to the directory structure. Examples:
   · "argument with friend" → emotional/relationship/conflict/
   · "job interview" + "anxiety" → factual/personal/milestone/ (past interviews) AND emotional/personal_growth/breakthrough/ (past success).
3. Contextual ranking: Sort files by timestamp (most recent first) or relevance score.
4. Memory injection: Insert top N summaries into the system prompt as a “historical context” block.

Example of automatic context injection:

```
[System] Relevant memories retrieved:
- 2026-07-14: Conflict with a close friend about future plans. Core issue: different life goals. 
  (Path: luffy_v1/emotional/relationship/conflict/evt_20260714_001.json)
- 2026-06-01: Successfully resolved a communication breakdown by writing a letter. This strategy was highly effective. 
  (Path: luffy_v1/experiential/learning/communication/practice/evt_20260601_005.json)
```

All retrieval MUST respect encryption boundaries; decryption occurs client-side after user authentication.
...
· Project end: Save under experiential/project/{project_id}/ with final proposal, key decisions, roles, and outcomes.
· Emotional/personal growth session: Save under emotional/personal_growth/ or emotional/relationship/ with core insights, emotion curve, and self-reflection conclusions.
...
All layers and path-archived memories MAY be exported as a single .mil file:
...
```json
{
  ...
  "dynamic_state": { /* ... */ },
  "path_archives": {
    "directory_tree": "/* JSON structure representing the logical tree with file paths as keys and memory metadata as values */"
  },
  "layers": {
    "layer1_event_log": [ /* ... */ ],
...
archive_memory(memory_object) Persist a memory summary to the appropriate path under the standardized tree. The client MUST determine the correct path based on memory_type, scene_type, etc.
query_path(psdl_id, memory_type, scene_type, filters) Return memory file entries (metadata and content, after decryption) from the specified path, optionally filtered by time range or keywords.
context_retrieve(psdl_id, current_message, context) Analyze the current interaction and return a set of relevant memory summaries from the path archive. Output format MUST include the path and memory_id of each entry.
...
Invalid mil_version 400 Reject: "Unsupported version"
Missing required field 400 Reject: "Missing field: [field_name]"
Invalid node_type 400 Reject: "Invalid node_type."
Invalid memory_type or scene_type for archive 400 Reject: "Unknown memory path category."
Decryption failure 401 Reject: "Invalid password or corrupted data"
Malformed JSON 400 Reject: "Invalid JSON syntax"
psdl_id mismatch in Dynamic State or path archive 409 Reject: "PSDL identity mismatch"
...
A minimal but complete export containing event log, semantic graph, meta-memory, dynamic state, and one archived memory file (encrypted content represented as a base64 blob).
